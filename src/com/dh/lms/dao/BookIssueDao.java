...
public class BookIssueDao {
    ...
    public void issuebook(Book book,User user) {
        ResultSet set = null;
        try{
            String sql = "insert into issue_book(user_id,book_id,issue_date,expected_return_date)values((select user_id from users where user_name=?),(select book_id from book where book_name=?),curdate(),DATE_ADD(curdate(),interval 7 day))";
